...
best_val_loss = -12.105501439239756  # from last checkpoint
best_epoch = 92  # from last checkpoint

# tensorboard
writer = SummaryWriter(log_dir=OUT_DIR, purge_step=START_EPOCH * train_loader_length)


# load previous checkpoint:
resume_path = "/mnt/raid/tbandyo/idp4vc_ws/SPECTRON_LOGS/VOICE_FILTER_DS/best_model_for_voice_filter_dataset.pth"
resume_state = torch.load(resume_path)
spk_enc.load_state_dict(resume_state["SE"])
sep_model.load_state_dict(resume_state["SPECTRON"])

# loop
if __name__ == "__main__":
    logging.info(f"\n----------------------------------------------\nResuming from last checkpoint at epoch {START_EPOCH}:\n----------------------------------------------\n\n")
